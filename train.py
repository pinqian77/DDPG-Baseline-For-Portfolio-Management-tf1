...
from network.ddpg_net import ActorNetwork, CriticNetwork
...
if __name__ == '__main__':
    ...
    # read data
    with h5py.File('history_stock_price.h5') as f:
        history_stock_price = f['stock_price'][...]
        timestamp = [s for s in f['timestamp']]
        abbreviations = [s for s in f['abbreviations']]
        features = [s for s in f['features']]
    ...
    with tf.variable_scope(variable_scope):
        ...
        stockactor = ActorNetwork(sess, config, action_bound=1)
        stockcritic = CriticNetwork(sess, config, num_actor_vars=stockactor.get_num_trainable_vars())

        agent = DDPG(env_training,sess,
                      actor = stockactor,
                      critic=stockcritic,
        ...
        agent.initialize(load_weights=False)
        agent.train()
